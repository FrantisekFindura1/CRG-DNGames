...
public class PlayScreen implements Screen {
    ...
    private Array<Rectangle> cloudImages;
    ...
    final int baseObstacleSpeed = 300;
    final int baseTimeToSpeed = 2000;
    int obstacleSpeed;
    int timeToSpeed;
    ...
    private void spawnCactusObstacle() {
     ...
     private void spawnCloud() {
      ...
      int randomCloudY = rand.nextInt(200) + 200;
          cloud.y = randomCloudY;
          cloud.x = 800 - 100;
      cloud.width = 85;
      cloud.height = 35;
      cloudImages.add(cloud); 
      lastCloudSpawnTime = TimeUtils.millis();
   }
    ...
    private void resetOnDeath(){
        ...
        for (Iterator<Rectangle> cloudIter = cloudImages.iterator(); cloudIter.hasNext(); ) {
            Rectangle cloud = cloudIter.next();
           cloudIter.remove();
        } 
    ...
    public void render(float delta) {
        ...
        Preferences prefs = Gdx.app.getPreferences("Dino_Highscore");
        ...
        batch.begin();
         ...
         for(Rectangle cloud: cloudImages) {
        Sprite cloudSize = new Sprite(cloudImage);
        cloudSize.setScale(1f);
        cloudSize.setPosition(cloud.x, cloud.y);
        cloudSize.setSize(cloud.width, cloud.height);
        cloudSize.draw(batch);
      }
  ...
  if(TimeUtils.millis() - lastCloudSpawnTime > rand.nextInt(10000) + 5000 - (obstacleSpeed * 1.5)){
      spawnCloud();
  }
            ...
            for (Iterator<Rectangle> cloudIter = cloudImages.iterator(); cloudIter.hasNext(); ) {
         Rectangle cloud = cloudIter.next();
         cloud.x -= ((50) + (obstacleSpeed *0.1)) * Gdx.graphics.getDeltaTime();
         if(cloud.x < 0 - 80) cloudIter.remove();
      }
        }
        else{
